...
class ofApp : public ofBaseApp{
	...
	public:
		...
		const float up = 1.0;
		const float e = 30.0f; // tiny e
		const float pushStrength = 50.0; // when user presses the keys this is how much force gets applied
		const float noiseScaleX = 0.001;
		const float noiseScaleY = 450.0;
		...
		glm::vec3 calcNormalForce(glm::vec3 gravity, glm::vec3 normal);
